...
public class NessusAdapterV1 extends AbstractAdapter<NessusAdapterContext, NessusAdapterConfig>
	...
	private class NessusWaitForScanStateSupport extends WaitForStateSupport<NessusAdapterContext, NessusAdapterConfig> {
		...
		protected String getCurrentState(NessusAdapterContext context) throws Exception {
			...
			ResponseEntity<String> response = context.getRestOperations().exchange(apiUrl, HttpMethod.GET, entity,
					String.class);
			if (!OK.equals(response.getStatusCode())) {
				throw new NessusRESTFailureException(response.getStatusCode(), response.getBody());
			}
